...
import android.content.pm.ActivityInfo;    //wuyixin modify here for show lunar when orientation = landscape
        ...
        for (; i < numCount; i++) {
            ...
            if (LunarUtils.showLunar() && service != null) {
                ...
                try {
					...
					int orientation = ((WindowManager) getContext().getSystemService(Context.WINDOW_SERVICE)).getDefaultDisplay().getOrientation();

                        //wuyixin modify here for show lunar when orientation = landscape
                        if(orientation == Surface.ROTATION_90 || orientation == Surface.ROTATION_270)  
                        {
                        		canvas.drawText(display, x, y + TOP_PADDING_MONTH_NUMBER, mMonthNumPaint);
                        }
                        else
                        {
