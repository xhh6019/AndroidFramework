...
//        mContext.requestWindowFeature(Window.FEATURE_INDETERMINATE_PROGRESS);
    ...
    if (DEBUG) Log.i("Mxhh","onActionBarItemSelected-----1");
        switch (itemId) {
			 
            case R.id.action_done:			
                if (EditEventHelper.canModifyEvent(mModel) || EditEventHelper.canRespond(mModel)) {
			if (DEBUG) Log.i("Mxhh","onActionBarItemSelected-----3");
                    if (mView != null && mView.prepareForSave()) {
			if (DEBUG) Log.i("Mxhh","onActionBarItemSelected-----4");
                        if (mModification == Utils.MODIFY_UNINITIALIZED) {
			if (DEBUG) Log.i("Mxhh","onActionBarItemSelected-----5");
                            mModification = Utils.MODIFY_ALL;
                        }
			if (DEBUG) Log.i("Mxhh","onActionBarItemSelected-----6");
                        mOnDone.setDoneCode(Utils.DONE_SAVE | Utils.DONE_EXIT);
                        mOnDone.run();
                    } else {
                     if (DEBUG) Log.i("Mxhh","onActionBarItemSelected-----7");
                        ...
                        && mOriginalModel != null && mView.prepareForSave()) {
                         if (DEBUG) Log.i("Mxhh","onActionBarItemSelected-----8");
                    saveReminders();
                    mOnDone.setDoneCode(Utils.DONE_EXIT);
                    mOnDone.run();
                } else {
             if (DEBUG) Log.i("Mxhh","onActionBarItemSelected-----9");
                    mOnDone.setDoneCode(Utils.DONE_REVERT);
                    mOnDone.run();
                }
		if (DEBUG) Log.i("Mxhh","onActionBarItemSelected-----10");
                break;
            case R.id.action_cancel:
		if (DEBUG) Log.i("Mxhh","onActionBarItemSelected-----11");
                mOnDone.setDoneCode(Utils.DONE_REVERT);
                mOnDone.run();
                break;
        }
...
if (DEBUG) Log.i("Mxhh","run-->3");
            if ((mCode & Utils.DONE_SAVE) != 0 && mModel != null
                    ...
                    if (DEBUG) Log.i("Mxhh","run-->4");
                int stringResource;
                if (!mModel.mAttendeesList.isEmpty()) {
					if (DEBUG) Log.i("Mxhh","run-->5");
                    if (mModel.mUri != null) {
						if (DEBUG) Log.i("Mxhh","run-->6");
                        stringResource = R.string.saving_event_with_guest;
                    } else {
                    if (DEBUG) Log.i("Mxhh","run-->7");
                        stringResource = R.string.creating_event_with_guest;
                    }
                } else {
                if (DEBUG) Log.i("Mxhh","run-->8");
                    if (mModel.mUri != null) {
						if (DEBUG) Log.i("Mxhh","run-->9");
                        stringResource = R.string.saving_event;
                    } else {
                    if (DEBUG) Log.i("Mxhh","run-->10");
                        stringResource = R.string.creating_event;
                    }
                }
				if (DEBUG) Log.i("Mxhh","run-->11");
            ...
            if ((mCode & Utils.DONE_EXIT) != 0) {
                ...
                if ((mCode & Utils.DONE_SAVE) != 0) {
					if (DEBUG) Log.i("Mxhh","run-->19");
                    if (mContext != null) {
						if (DEBUG) Log.i("Mxhh","run-->20");
                        long start = mModel.mStart;
                        long end = mModel.mEnd;
                        if (mModel.mAllDay) {
							if (DEBUG) Log.i("Mxhh","run-->21");
	...
	if (DEBUG) Log.i("Mxhh","test!!");
	mModel.mTitle=getResources().getString(R.string.what_label_default);
