...
public class VideoCamera extends ActivityBase
    ...
    private static final String[] OTHER_SETTING_KEYS = {
                    ...
                    CameraSettings.KEY_PHONE_MEMORY,
                    //CameraSettings.KEY_PHONE_SOUND,
                    CameraSettings.KEY_POWER_MODE
        };
	//save image internal memory and external memory
	private static final String[] OTHER_SETTING_KEYS1 = {
		//CameraSettings.KEY_PHONE_MEMORY
                  CameraSettings.KEY_PHONE_SOUND
	};
    ...
    private static  boolean stop_addvideo = true;
    ...
    private String mFlashMode;//flash mode
    ...
    private BroadcastReceiver mScreenOnReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            {
                mScreenLock = false;
				Log.v(TAG, "mPreviewing: " + mPreviewing);
                if(mPreviewing&&(mFlashMode.equals(Parameters.FLASH_MODE_TORCH)))//
                {
    ...
    private class MyBroadcastReceiver extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (action.equals(Intent.ACTION_MEDIA_EJECT)) {
				...
				if(externalStoragePath.equals(path) && (Storage.store_memory == 1))
				{
  ...
  //add xhh for feedback ledFlashButton
    private void sendABroadcastToLedFlashButton(Context context){
    	Intent mintent=new Intent();
    	mintent.setAction("com.android.systemui.statusbar.phone.LedFlashButton");
    	context.sendBroadcast(mintent);
    	Log.i("Mxhh", "sended in Videocamera");
    }
    ...
    public void onCreate(Bundle icicle) {
        ...
        //add xhh
        sendABroadcastToLedFlashButton(this);
        
		//support for internal memory
		//Storage.initStorageParmeter();
	...
	//support for internal memory
	Storage.store_memory = CameraSettings.readPreferredStoreMemory(mPreferences);
	//support for internal memory
	Storage.initStorageParmeter();
        ...
        //play sound or not
        mplaySound = CameraSettings.readPreferredSound(mPreferences);
        mCameraDevice.enableShutterSound(mplaySound);
    }
    ...
    private void initializeIndicatorControl() {
		...
		//save image internal memory and external memory
		otherSettingKeys.put(1,OTHER_SETTING_KEYS1);
    ...
    public void onThumbnailClicked(View v) {
        //@iris20130527 for enter thumbnail without flash
        //if(mParameters.getFlashMode().equals(Parameters.FLASH_MODE_TORCH))
        //{
        ...
        if(mIsVideoCaptureIntent){
           if (android.os.Environment.getExternalStorageState().equals(  
        	    android.os.Environment.MEDIA_MOUNTED)) {
                 mVideoFilename = "/mnt/sdcard/DCIM/Camera"+ '/' + filename;
           }else{
                  mVideoFilename = "/mnt/emmc" + '/' + filename;
           }
        }
...
*/
    ...
    private void getThumbnail() {
        if (mCurrentVideoUri != null) {
            ...
            if (videoFrame != null) {
				...
				//save image internal memory and external memory
				if(Storage.store_memory == 0)
				{
					mThumbnail.saveTo(new File(getFilesDir(), Thumbnail.LAST_IVTHUMB_FILENAME));
				}
				else if(Storage.store_memory == 1)
				{
					mThumbnail.saveTo(new File(getFilesDir(), Thumbnail.LAST_EVTHUMB_FILENAME));
				}
    ...
    private void initThumbnailButton() {
		...
		if(Storage.store_memory == 0)
		{
			mThumbnail = Thumbnail.loadFrom(new File(getFilesDir(), Thumbnail.LAST_IVTHUMB_FILENAME));
		}
		else if(Storage.store_memory == 1)
		{
			mThumbnail = Thumbnail.loadFrom(new File(getFilesDir(), Thumbnail.LAST_EVTHUMB_FILENAME));
		}
    }
    ...
    private void updateThumbnailButton() {
        if ((mThumbnail == null || !Util.isUriValid(mThumbnail.getUri(), mContentResolver))
              ...
              //mThumbnail = Thumbnail.getLastThumbnail(mContentResolver);
               mThumbnail = Thumbnail.gainLastVideoThumbnail(mContentResolver);
        }
        if (mThumbnail != null) {
            mThumbnailView.setBitmap(mThumbnail.getBitmap());
            mThumbnail.getBitmap().recycle();
        } else {
 ...
 //       if(mParameters.isPowerModeSupported()) {
		//save image internal memory and external memory
		 if (!mIsVideoCaptureIntent){
            int lastStore = Storage.store_memory;       
            int newStore = CameraSettings.readPreferredStoreMemory(mPreferences);   

			if(lastStore != newStore)
			{
				Storage.store_memory = newStore;
				CameraSettings.writePreferredStoreMemory(mPreferences,newStore);
				Storage.initStorageParmeter();
				if(Storage.store_memory == 0)
				{
					mThumbnail = Thumbnail.loadFrom(new File(getFilesDir(), Thumbnail.LAST_IVTHUMB_FILENAME));
        ...
        //play sound
        int playSound = CameraSettings.readPreferredSound(mPreferences);
        if(mplaySound != playSound)
        {
    ...
    //iris for videocamera flash mode 
    @Override
