...
public class CallFeaturesSetting extends PreferenceActivity
    ...
    private static final String BUTTON_IP_CALL_KEY   = "ipcall_dial_settings_cdma";
    ...
    //wuyixin modify here for extra param no same as SUBSCRIPTION_KEY,and read error.
    ...
    private PreferenceScreen mIPCallSetting; 
    ...
    public boolean onPreferenceTreeClick(PreferenceScreen preferenceScreen, Preference preference) {
        ...
        if (preference == mSubMenuVoicemailSettings) {
            ...
            return true;
        }else if(preference == mIPCallSetting) {    //By Michael.Chan @ Jun.21 2013 
                            ...
                            new DialogInterface.OnClickListener() {
                                public void onClick(DialogInterface dialog, int which) {
                                    ...
                                    if (TextUtils.isEmpty(ip_prefix)) {
                                        mIPCallSetting.setSummary(R.string.ipcall_sub_summery);
                                    } else {
                                        mIPCallSetting.setSummary(edit.getText());
                                    }
                                    onResume();
                                }
    ...
    protected void onCreate(Bundle icicle) {
        ...
        if (mDisplayHomeLocation != null ) {
		 ...
		 //By Michael.Chan @ Jun.21 2013  BEGIN
		 mIPCallSetting = (PreferenceScreen) findPreference(BUTTON_IP_CALL_KEY);
		 mIPCallSetting.setOnPreferenceChangeListener(this);
				if (mIPCallSetting != null) {
					if (true) { // TODO: "true" need change to feature query.
						String ip_prefix = Settings.System.getString(getContentResolver(), Settings.System.IPCALL_PREFIX[0]);
						if (TextUtils.isEmpty(ip_prefix)) {
							mIPCallSetting.setSummary(R.string.ipcall_sub_summery);
						} else {
							mIPCallSetting.setSummary(ip_prefix);
						}
					} else {
						getPreferenceScreen().removePreference(mIPCallSetting);
						mIPCallSetting = null;
					}
				}
			//By Michael.Chan @ Jun.21 2013  END
