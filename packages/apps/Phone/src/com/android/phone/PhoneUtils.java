...
public class PhoneUtils {
    ...
    static void turnOnSpeaker(Context context, boolean flag, boolean store) {
        ...
        final PhoneApp app = PhoneApp.getInstance();
        if(app.mCM.getModemIndex() == 1){
            if(flag){
                app.mCM.setAudioDevice(CallManager.QSC1110AudioDeviceType.SPEAKER);
            }
            else {
                if(app.isHeadsetPlugged()) {
                    app.mCM.setAudioDevice(CallManager.QSC1110AudioDeviceType.HEADSET);
                }
                else {
                    app.mCM.setAudioDevice(CallManager.QSC1110AudioDeviceType.HANDSET);
                }                   
            }
        }
        else {
	...
	static void turnOnAudioDevice(Phone phone, PhoneApp app) {
       	if (phone != null && phone.getModemIndex() == 1) {
			if(app.mCM.getAudioDevice() == CallManager.QSC1110AudioDeviceType.SPEAKER){
				phone.setAudioDevice(CallManager.QSC1110AudioDeviceType.SPEAKER);
			}
			else {
				if(app.isHeadsetPlugged()) {
					phone.setAudioDevice(CallManager.QSC1110AudioDeviceType.HEADSET);
				}
				else {
					phone.setAudioDevice(CallManager.QSC1110AudioDeviceType.HANDSET);
				}					
			}
		}
	}
    ...
    static void restoreSpeakerMode(Context context) {
        ...
        if(PhoneApp.getInstance().mCM.getModemIndex() == 1){
    ...
    static boolean isSpeakerOn(Context context) {
        ...
        final PhoneApp app = PhoneApp.getInstance();
		log("turnOnNoiseSuppression: " + app.mCM.getModemIndex());
        if(app.mCM.getModemIndex() == 1){
    ...
    private static void setMuteInternal(Phone phone, boolean muted) {
        ...
        if (routeToAudioManager && phone.getModemIndex() !=1) {
    ...
    static boolean getMute() {
        ...
        if (routeToAudioManager && app.mCM.getModemIndex() != 1) {
            AudioManager audioManager =
