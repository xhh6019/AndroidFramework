...
public class InCallScreen extends Activity
	...
	private void internalSwapToSub1() {
		if (DBG) log("internalSwapToSub1()...");
		hideDialpadInternal(true); 
		mDialer.clearDigits();
		if (Switch_sub !=1){
			try{  
				if(mApp.getPhone(0).getBackgroundCall().getState()==Call.State.HOLDING){
					mApp.getPhone(0).getBackgroundCall().getPhone().switchHoldingAndActive();
				}
				mApp.getPhone(1).setAudioDevice(CallManager.QSC1110AudioDeviceType.AUDIO_SUB1);
				Switch_sub = 1;
	...
	private void internalSwapToSub2() {
		if (DBG) log("internalSwapToSub2()...");
		hideDialpadInternal(true); 
		mDialer.clearDigits();
		if (Switch_sub !=2){
			try{  
				...
				mApp.getPhone(1).setAudioDevice(CallManager.QSC1110AudioDeviceType.AUDIO_SUB2);
				Switch_sub = 2;
			}catch(CallStateException e){
			}
		}
