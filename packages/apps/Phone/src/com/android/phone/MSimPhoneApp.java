...
public class MSimPhoneApp extends PhoneApp {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (action.equals(Intent.ACTION_AIRPLANE_MODE_CHANGED)) {
                ...
                if (streamType == AudioManager.STREAM_VOICE_CALL) {
                    ...
                    for (int i = 0; i < TelephonyManager.getDefault().getPhoneCount(); i++) {
                        if(getPhone(i).getModemIndex() == 1) {
                            getPhone(i).setSoundLevel(newVolLevel);
                        }
                    }
                }
            }else {
    ...
    public void modemSwitch()
    {  
        Log.w(LOG_TAG, "modemSwitch");
    	  getPhone(1).modemSwitch();
    }
	
    /* luxx@20130513 modify here for modem USB connect with PC for debug log or calibration */
    public void modemCalibrate()
    {  
        Log.w(LOG_TAG, "modemCalibrate");
    	  getPhone(1).modemCalibrate();
    }
