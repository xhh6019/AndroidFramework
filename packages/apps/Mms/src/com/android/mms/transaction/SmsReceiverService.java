...
//        }
        ...
        if (c != null) {
            try {
                if (c.moveToFirst()) {
                    ...
                    Log.d(TAG, "subscription in database =" + c.getInt(SEND_COLUMN_SUB_ID) + ", subscription=" + subscription);
    ...
    private void handleSmsSent(Intent intent, int error) {
        ...
        if (mResultCode == Activity.RESULT_OK) {
            ...
            if (sendNextMsg) {
                if (TelephonyManager.getDefault().isMultiSimEnabled()) {
                    sendFirstQueuedMessage(intent.getIntExtra(SUBSCRIPTION_KEY, 0));
                } else {
                    sendFirstQueuedMessage();
                }
            }
    ...
    private void handleCbSmsReceived(Intent intent, int error) {
        ...
        /* This check is not necessary in current design.
