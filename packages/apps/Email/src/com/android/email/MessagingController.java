...
public class MessagingController implements Runnable {
            ...
            public void messageRetrieved(Message message) {
                try {
                    ...
                    try {
                        ...
                        // TODO: maybe need add function for feature sync size.
    ...
    private SyncResults synchronizeMailboxGeneric(final Account account, final Mailbox mailbox)
        ...
        // 9. Refresh the flags for any messages in the local store that we didn't just download. fetch FLAGS
        FetchProfile fp = new FetchProfile();
        fp.add(FetchProfile.Item.FLAGS);
        ...
        put("loadMessageForViewRemote", listener, new Runnable() {
            public void run() {
                try {
                    ...
                    boolean pop3Message = false;
                    if (remoteMessage instanceof Pop3Message) {
                        pop3Message = true;
                    }
                    remoteMessage.setNeedSyncSize(Utility.getAccountSyncSize(mContext, message.mAccountKey));
                    FetchProfile fp = new FetchProfile();
                    if (EmailContent.Message.FLAG_LOADED_SYNC_SIZE_COMPLETE == flag && !pop3Message) {
                        fp.add(FetchProfile.Item.BODY_SANE);
                    } else {
                        fp.add(FetchProfile.Item.BODY);
                    }
                    ...
                    copyOneMessageToProvider(remoteMessage, account, mailbox,
                            pop3Message ? EmailContent.Message.FLAG_LOADED_COMPLETE : flag);
