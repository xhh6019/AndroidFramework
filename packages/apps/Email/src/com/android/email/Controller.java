...
//            }
    ...
    public void loadMessageForView(final long messageId, final int flag) {
        ...
        if (service != null) {
            ...
            // MessagingController implementation. For imap account.
            Utility.runAsync(new Runnable() {
                public void run() {
    ...
    private class ServiceCallback extends IEmailServiceCallback.Stub {
        ...
        @Override
        public void loadMoreStatus(long messageId, int statusCode, int progress) throws RemoteException {
            MessagingException result = mapStatusToException(statusCode);
            switch (statusCode) {
                case EmailServiceStatus.SUCCESS:
                    progress = 100;
    ...
    private final IEmailServiceCallback.Stub mCallbackProxy =
        ...
        @Override
        public void loadMoreStatus(final long messageId, final int statusCode, final int progress) {
            broadcastCallback(new ServiceCallbackWrapper() {
                @Override
                public void call(IEmailServiceCallback cb) throws RemoteException {
                    cb.loadMoreStatus(messageId, statusCode, progress);
                }
            });
        }
    };
