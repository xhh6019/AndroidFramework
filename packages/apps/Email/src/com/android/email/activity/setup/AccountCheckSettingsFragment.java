...
public class AccountCheckSettingsFragment extends Fragment {
        ...
        if (mAttached) {
            ...
            switch (newState) {
                case STATE_CHECK_OK:
                    ...
                    // 2. exit self
                    try { // if the fragment saveInstanceState, will throw the IllegalStateException
                        fm.popBackStack();
                    } catch (IllegalStateException e) {
                        Log.w(TAG, "The FragmentManager popBackStack catch the IllegalStateException");
                    }
                ...
                case STATE_CHECK_SHOW_SECURITY:
                    ...
                    if (fm.findFragmentByTag(SecurityRequiredDialog.TAG) == null) {
                        ...
                        try {
                            ...
                            Log.w(TAG, "The FragmentManager beginTransaction catch the IllegalStateException");
                        }
                    }
                    break;
    ...
    private class AccountCheckTask extends AsyncTask<Void, Integer, MessagingException> {
        ...
        protected void onPostExecute(MessagingException result) {
            if (isCancelled()) return;
            if (result == null) {
                // If rotate the screen, the SetupDate will init a new account.
                // And we need reset the account as mAccount.
