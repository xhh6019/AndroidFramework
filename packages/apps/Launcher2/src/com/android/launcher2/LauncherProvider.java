...
public class LauncherProvider extends ContentProvider {
    ...
    public int update(Uri uri, ContentValues values, String selection, String[] selectionArgs) {
    	if (uri.toString().equals("content://com.android.launcher2.settings/resetdb")) {
    ...
    private static class DatabaseHelper extends SQLiteOpenHelper {
            ...
            if (!convertDatabase(db)) {
                ...
                try {
					...
					if (!currentTheme.equals("default")) {
						...
						ZipEntry hotseatEntry = themeSrc.getEntry(hotseat_config);
						if (hotseatEntry != null) {
							stream = themeSrc.getInputStream(hotseatEntry);							
							loadFavoritesByPath(db, stream);
							android.util.Log.e("filePathInThemeHotSeat", "use theme"); 
						}		
						themeSrc.close();
					}
					else
					...
					android.util.Log.e("filePathInThemeHotSeat Exception", "" + e); 
                }				
            }
        }
			    ...
			    while (eventType != XmlPullParser.END_DOCUMENT) { 					
					switch (eventType) {
						case XmlPullParser.START_TAG:
							...
							if (parser.getAttributeValue(launcherXMLNS, "container") != null)
		                        container = Long.valueOf(parser.getAttributeValue(launcherXMLNS, "container"));

		                    String screen = parser.getAttributeValue(launcherXMLNS, "screen");
		                    String x = parser.getAttributeValue(launcherXMLNS,"x");
		                    String y = parser.getAttributeValue(launcherXMLNS,"y");
							String span_x = parser.getAttributeValue(launcherXMLNS,"spanX");
		                    String span_y = parser.getAttributeValue(launcherXMLNS,"spanY");
							String clsName = parser.getAttributeValue(launcherXMLNS,"className");
							String pkgName = parser.getAttributeValue(launcherXMLNS,"packageName");
		                    ...
		                    if (TAG_FAVORITE.equals(name)) {
		                        long id = addAppShortcutExt(db, values, null, pkgName, clsName, packageManager, intent);
		                        added = id >= 0;
		                        ...
		                        added = addAppWidgetExt(db, values, null, pkgName, clsName, span_x, span_y, packageManager);
		                    }
		                    if (added) i++;
        ...
        private int loadFavorites(SQLiteDatabase db, int workspaceResourceId) {
            ...
            try {
                ...
                while (((type = parser.next()) != XmlPullParser.END_TAG ||
                    ...
                    if (TAG_FAVORITE.equals(name)) {
                        long id = addAppShortcutExt(db, values, a, null, null, packageManager, intent);
                        added = id >= 0;
                    } else if (TAG_SEARCH.equals(name)) {
                        added = addSearchWidget(db, values);
                    } else if (TAG_CLOCK.equals(name)) {
                        ...
                        added = addAppWidgetExt(db, values, a, null, null, null, null, packageManager);
                        ...
                        while ((type = parser.next()) != XmlPullParser.END_TAG ||
                            ...
                            if (TAG_FAVORITE.equals(folder_item_name) && folderId >= 0) {
                                long id =
                                    addAppShortcutExt(db, values, ar, null, null, packageManager, intent);
                                if (id >= 0) {
                                    folderItems.add(id);
                                }
        ...
        private long addAppShortcutExt(SQLiteDatabase db, ContentValues values, TypedArray a, String pkgName, String clsName,  
        ...
        private boolean addAppWidgetExt(SQLiteDatabase db, ContentValues values, TypedArray a, String pkgName,
                String clsName, String strSpanX, String strSpanY, PackageManager packageManager) {
