...
public class Controller
    ...
    public void startVoiceSearch() {
        ...
        try {
            ...
            Log.w(LOGTAG,"Voice search did not exit!");
        }
    }
    ...
    public void updateMenuState(Tab tab, Menu menu) {
        ...
        if (!FeatureQuery.FEATURE_BROWSER_UA_SELECTION) {
            //desktop item is the member of LIVE_MENU group
            uaSwitcher.setVisible(false);
        }
        menu.setGroupVisible(R.id.SNAPSHOT_MENU, !isLive);
        if (FeatureQuery.FEATURE_BROWSER_HISTORY_MENU) {
            //history and snapshots item are the members of COMBO menu group,
            //so if show history item, only visible snapshots item.
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        switch (item.getItemId()) {
            ...
            case R.id.exit_menu_id:
                ...
                Log.w(LOGTAG,"Get Monkeytest ret="+ret);
                ...
                if (values != null) {
                    new AsyncTask<Tab, Void, Long>() {
                        ...
                        protected void onPostExecute(Long id) {
                            if (id == null) {
					//add xhh for toast Sdcard
					toastNotSdCard();
					//move to toastNotSdCard();
    ...
    private void closeAllTabs() {
        ...
        //set new tab to current tab and remove previous current tab
        mTabControl.setCurrentTab(newTab);
        removeTab(current);
        closeAllTabs();
    }
...
// add xhh for toast SDcard
    private void toastNotSdCard(){
		if (!(Environment.getExternalStorageState().equals(Environment.MEDIA_MOUNTED)) ){
			Toast.makeText(mActivity, R.string.snapshot_failed_sdCard,
                                        Toast.LENGTH_SHORT).show();
			}else{
			Toast.makeText(mActivity, R.string.snapshot_failed,
                                        Toast.LENGTH_SHORT).show();
