...
public class MediaAppWidgetProvider extends AppWidgetProvider {
    ...
    private void defaultAppWidget(Context context, int[] appWidgetIds) {
        ...
        CharSequence StateText = null;
        if (status.equals(Environment.MEDIA_SHARED) || status.equals(Environment.MEDIA_UNMOUNTED) ) {
            if (android.os.Environment.isExternalStorageRemovable()) {
                StateText = res.getText(R.string.sdcard_busy_title);
            } else {
                StateText = res.getText(R.string.sdcard_busy_title_nosdcard);
            }
        } else if (status.equals(Environment.MEDIA_BAD_REMOVAL) || status.equals(Environment.MEDIA_REMOVED)) {
            if (android.os.Environment.isExternalStorageRemovable()) {
                StateText = res.getText(R.string.sdcard_missing_title);
            } else {
                StateText = res.getText(R.string.sdcard_missing_title_nosdcard);
            }
        } else {
            StateText = res.getText(R.string.widget_initial_text);
        }

        views.setViewVisibility(R.id.title, View.GONE);
        views.setTextViewText(R.id.artist, StateText);
