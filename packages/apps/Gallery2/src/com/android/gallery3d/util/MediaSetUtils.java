...
public class MediaSetUtils {
    ...
    public static final int EMMC_CAMERA_BUCKET_ID = GalleryUtils.getBucketId(
    ...
    private static final Path[] EMMC_CAMERA_PATHS = {
            Path.fromString("/local/all/" + EMMC_CAMERA_BUCKET_ID),
            Path.fromString("/local/image/" + EMMC_CAMERA_BUCKET_ID),
            Path.fromString("/local/video/" + EMMC_CAMERA_BUCKET_ID)};			
    public static boolean isCameraSource(Path path) {
        return CAMERA_PATHS[0] == path || CAMERA_PATHS[1] == path
                || CAMERA_PATHS[2] == path
                ||EMMC_CAMERA_PATHS[0] == path || EMMC_CAMERA_PATHS[1] == path
                || EMMC_CAMERA_PATHS[2] == path;
    }
