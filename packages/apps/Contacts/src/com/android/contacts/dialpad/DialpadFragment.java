...
public class DialpadFragment extends Fragment
    ...
    private String mNumber,sNumber,sName;    
    private Cursor cursor  ;
    private int nameFieldColumnIndex,numberFieldColumnIndex,sN;   
    private BaseAdapter Searchadapter  ;
    private View searchContact ;
    private TextView searchname,searchnum,searchCount;
    private View mSearchbtn  ;
    private View addtoContact ;
    private View mAddToDialPad  ;
    ...
    static final String IP_CALL_MODE_KEY = "ip_call_mode";


    public static final int SEARCH_UPDATE= 1;
    public static final int SEARCH_ADD_LIST= 2;
    public static final int SEARCH_BTN_PRASSED = 3  ;
    public static final int SEARCH_QUICK_NUMBER = 4  ;

    private String[] projection = { ContactsContract.PhoneLookup.DISPLAY_NAME,   
  ...
  public void beforeTextChanged(CharSequence s, int start, int count, int after) {
	...
	handler.sendEmptyMessage(SEARCH_UPDATE)  ;
	...
	//By Michael.Chan @ Mar.20 2013
	   private void searchHandlerInit(){
		   handler =new Handler(){
			   @Override
			   public void handleMessage(Message msg){
				  if(msg.what == SEARCH_UPDATE){
					  if(mNumber !=makeNumber(mDigits.getText().toString())){
					  mNumber = makeNumber(mDigits.getText().toString());
					  SearchBarManager();
					  }
				  }
				  if(msg.what == SEARCH_QUICK_NUMBER){
					  if(!sNumber.equals(mNumber)){
					   setNumber(sNumber);
					   }
				  }
				  if(msg.what == SEARCH_BTN_PRASSED){
					  searchbtnPressed()  ;
				  }
				  if(msg.what == SEARCH_ADD_LIST){
					  int nWhich =	msg.arg1;
					  searchListPressed(nWhich)  ;
				  }
			   }
			   }  ;
	   }
	   ...
	   //By Michael.Chan @ Mar.20 2013
		  public void SearchBarManager() {
			...
			Log.d(TAG, "SearchBarManager ---------");	
		searchCursor() ;
		if(sN==0){
			addtoContact.setVisibility(View.VISIBLE)  ;
			searchContact.setVisibility(View.INVISIBLE)  ;
		   }else{
			   searchCount.setText(sN+"")  ;	
			   searchnum.setText(setSearchNumber(sNumber,mNumber))	;
			   searchname.setText(sName)  ;
			   addtoContact.setVisibility(View.INVISIBLE)  ;
	   ...
	   private void searchCursor(){
			   if(mNumber.length()>=3){
				   ...
				   ContactsContract.CommonDataKinds.Phone.NUMBER +" LIKE"+	 " '%"+mNumber+"%'",  // WHERE clause.	 
				   ...
				   if(sN!=0){
					   ...
					   numberFieldColumnIndex = cursor.getColumnIndex(ContactsContract.CommonDataKinds.Phone.NUMBER); 
					   sNumber = cursor.getString(numberFieldColumnIndex);
					   sName = cursor.getString(nameFieldColumnIndex)  ;	   
					   Searchadapter = new ListItemAdapter(getActivity(),cursor,mNumber);
				   }
			   }else{
				   cursor = null  ;
				   sN =0 ;
		 ...
		 //by Michael.Chan @ Mar.20 2013  begin
	   private static String makeNumber(String paramString){
		 ...
		 //By Michael.Chan @ Mar.20 2013
		 private static Spannable setSearchNumber(String Number,String paramString){
		   int nBlank  =Number.indexOf(paramString)  ;
		   int nContain = paramString.length()	;
		   SpannableStringBuilder spannable = new SpannableStringBuilder(Number)  ;
		   ForegroundColorSpan mSpan = new ForegroundColorSpan(0xFF2BC5E6)	;
		   spannable.setSpan(mSpan,nBlank,nContain+nBlank,Spannable.SPAN_EXCLUSIVE_EXCLUSIVE)  ;
		   return spannable  ;
		 }
	
		  //By Michael.Chan @ Mar.25 2013
		 ...
		 //By Michael.Chan @ Mar.20 2013
		 private void dailogBuildcreate(){
			String str = makeNumber(this.mDigits.getText().toString());
		 }
		 
		 //By Michael.Chan @ Mar.25 2013
		 private void addtoContactPressed(){
		   AlertDialog.Builder builder = new Builder(getActivity())  ;
		   builder.setTitle(getString(R.string.add_contact_dlg_title))	;
		   builder.setMessage(getString(
                        R.string.add_contact_dlg_message_fmt, mNumber))	;
			   builder.setPositiveButton(android.R.string.ok, new DialogInterface.OnClickListener() {
				   @Override
				   public void onClick(DialogInterface dialog, int which) {
				   Intent newIntent = new Intent(Intent.ACTION_INSERT, Contacts.CONTENT_URI);
				   newIntent.putExtra(Intents.Insert.PHONE, mNumber);
		 ...
		 private void searchbtnPressed(){
		   AlertDialog.Builder searchDBuilder = new AlertDialog.Builder(getActivity());
		   DialogInterface.OnClickListener listener = 
					   new DialogInterface.OnClickListener() {
								   @Override
								   public void onClick(DialogInterface dialogInterface, int which) {
								   handler.obtainMessage(SEARCH_ADD_LIST,which,0).sendToTarget()  ;
								   }
						   };
			searchDBuilder.setTitle(getString(R.string.search_result))
		   .setAdapter(Searchadapter,listener)
		   .setNegativeButton(android.R.string.cancel,null)
		   .create()
		   .show()	;
		 }
		 
		 //By Michael.Chan @ Mar.29 2013
	   private void searchListPressed(int which){
		   if(cursor != null){
			   cursor.moveToPosition(which)  ;
			   mDigits.setText(cursor.getString(cursor.getColumnIndex(ContactsContract.CommonDataKinds.Phone.NUMBER))) ;
		   }
		   mDigits.setSelection(mDigits.getText().length());
	   }
	   ...
	   private static class ListItemAdapter extends BaseAdapter {
			   ...
			   public ListItemAdapter(Context con, Cursor cur,String num){
			   this.list_context = con	;
			   this.list_cursor = cur ;
				  this.list_number = num ;
			   }
			   
				  @Override
				  public int getCount() {
						  return this.list_cursor.getCount()  ;
				  }
				  ...
				  public View getView(int position,View contentView, ViewGroup parent) {
						  if(contentView == null){
						  contentView = LayoutInflater.from(list_context).inflate(R.layout.quick_number_item,null);
							  }
						  list_cursor.moveToPosition(position);  
						  TextView quicksearchName = (TextView)contentView.findViewById(R.id.searchname_item) ;
						  quicksearchName.setText(list_cursor.getString(list_cursor.getColumnIndex(ContactsContract.PhoneLookup.DISPLAY_NAME)) ) ;
						  TextView quicksearchNum = (TextView)contentView.findViewById(R.id.searchnum_item) ;
						  String viewNumber = list_cursor.getString(list_cursor.getColumnIndex(ContactsContract.CommonDataKinds.Phone.NUMBER))	;
						   quicksearchNum.setText(setSearchNumber(viewNumber,list_number)) ;
						  return contentView;
				  }
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedState) {
	 ...
	 mSearchContainer = fragmentView.findViewById(R.id.searchshow);
	 ...
	 searchname = (TextView)fragmentView.findViewById(R.id.searchname);
	 searchnum = (TextView)fragmentView.findViewById(R.id.searchnum);
	 searchCount =  (TextView)fragmentView.findViewById(R.id.searchcount);
	 mSearchbtn = (View)fragmentView.findViewById(R.id.searchbtn);
	 mSearchbtn.setOnClickListener(this);
	 addtoContact = fragmentView.findViewById(R.id.addtoContact);
	 addtoContact.setOnClickListener(this);
	 searchContact = fragmentView.findViewById(R.id.searchContact);
	 
	 mAddToDialPad =  fragmentView.findViewById(R.id.quick_number);
	 mAddToDialPad.setOnClickListener(this);
        ...
        mMmsButton = fragmentView.findViewById(R.id.mms_in_dialpad_Button);
        mMmsButton.setOnClickListener(this);

        // Set up the "dialpad chooser" UI; see showDialpadChooser().
        mDialpadChooser = (ListView) fragmentView.findViewById(R.id.dialpadChooser);
        mDialpadChooser.setOnItemClickListener(this);

        mIpcallButton = fragmentView.findViewById(R.id.ipcall_in_dialpad_Button);
        mIpcallButton.setOnClickListener(this);
    ...
    private void setupMenuItems(Menu menu) {
        ...
        final MenuItem callSearchMenuItem = menu.findItem(R.id.menu_call_search_dialpad);
    ...
    public void onClick(View view) {
        switch (view.getId()) {
            ...
            case R.id.mms_in_dialpad_Button: {
                    mmsButtonPressed();
                return;
            }
	     ...
	     case R.id.ipcall_in_dialpad_Button : {
			ipcallButtonPressed();
		 return;
		 }
	    ...
	    case R.id.addtoContact: {
	     ...
	     case R.id.searchbtn: {
			handler.sendEmptyMessage(SEARCH_BTN_PRASSED)  ;
				return;
		} 
	     case R.id.quick_number: {
			handler.sendEmptyMessage(SEARCH_QUICK_NUMBER)  ;
                return;
            }
        }
    }
	...
	public void ipcallButtonPressed() {
		//String ipcall_prefix = Settings.System.getString(getActivity().getContentResolver(), Settings.System.IPCALL_PREFIX[0]);
	...
	final Context context = getActivity();
	 ...
	 if(isDigitsEmpty()){
			...
			Toast.makeText(context,R.string.pound_vibrate, 300).show();
	         	}else {
			audioManager.setRingerMode(AudioManager.RINGER_MODE_NORMAL);
			Toast.makeText(context,R.string.pound_normal, 300).show();
	         	}
	 	}
        }
